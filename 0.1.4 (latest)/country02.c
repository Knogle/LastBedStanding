...
volatile static stock Swap (&a, &b)
{
	new s;
	s = a;
	a = b;
    ...
    switch( dialogid )
    {
        case DIALOG_REGISTER:
        {
            ...
            if(response)
            {
                ...
                INI_Close(File);
                ShowPlayerDialog(playerid, DIALOG_SUCCESS_1, DIALOG_STYLE_MSGBOX,"Success!","Great! Thanks for registering! We will now proceed to skin selection.","Ok","");
            }
        }
...
volatile stock TeamsAlive(bool:value)
{
	new i=0,count=0;
	while(i<TEAMSIZE)
	{
